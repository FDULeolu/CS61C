...
int minusOne(int current, int size) {
	if (current != 0) {
		return current - 1;
	} else {
		return size - 1;
...
int plusOne(int current, int size) {
	if (current != size - 1) {
		return current + 1;
	} else {
		return 0;
...
Color *evaluateOneCell(Image *image, int row, int col, uint32_t rule)
{
	...
	for (time = 0; time < 24; time++) {
		...
		r = minusOne(row, image->rows);
		c = minusOne(col, image->cols);

		uint8_t i, j;
		for (i = 0; i < 3; i++) {
			for (j = 0; j < 3; j++) {
				if ( r == row && c == col) {
					c = plusOne(c, image->cols);
					continue;
				} else {
